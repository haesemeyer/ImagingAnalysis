...
def make_linearModel(act_in, act_out):
    """
...
if __name__ == "__main__":
    ...
    for i in [1, 3]:
        ...
        n = response_names[i]
        model_results[n] = ModelResult(tg_out, betas, f[::-1], "CUBIC", nl_params)
    ...
    for i in [0, 2]:
        output = standardize(trial_average(region_results["Rh6"].full_averages[:, i]))
        model = make_offInhibited_Rh6_model(tg_on, rh6_slow_off, output, filter_length)
        start = pm.find_MAP(model=model)
        with model:
            trace = pm.sample(n_samples, start=start)
        betas = np.array([np.mean(trace["beta_on"]), np.mean(trace["beta_off"])])
        f = np.mean(trace["f"], 0).ravel()
        ...
        print("Type {0} coefficients: {1}".format(i, betas))
        n = response_names[i]
        model_results[n] = ModelResult(off_inh_out, betas, f[::-1], "CUBIC", nl_params)
        model_results[n].trace_object = trace
    ...
    for i in [4]:
        output = standardize(trial_average(region_results["Rh6"].full_averages[:, i]))
        model = make_onInhibited_Rh6_model(rh6_slow_on, tg_off, output, filter_length)
        start = pm.find_MAP(model=model)
        with model:
            trace = pm.sample(n_samples, start=start)
        betas = np.array([np.mean(trace["beta_on"]), np.mean(trace["beta_off"])])
        ...
        print("Type {0} coefficients: {1}".format(i, betas))
        n = response_names[i]
        model_results[n] = ModelResult(on_inh_out, betas, f[::-1], "CUBIC", nl_params)
        model_results[n].trace_object = trace
